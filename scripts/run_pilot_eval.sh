...
run_eval() {
  ...
  for ((shard=0; shard<SHARDS; shard++)); do
    OMP_NUM_THREADS=2 CUDA_VISIBLE_DEVICES="$device" "$PYTHON" src/tick_inference.py \
      ...
      --adapter "$GRID_ROOT/${group}_lr_${lr}/adapter" --group "$group" --cache \
      --split validation --limit "$LIMIT" --num-shards "$SHARDS" --shard-index "$shard" \
      --output "$output/shards/${shard}.jsonl" > "$output/shards/${shard}.log" 2>&1 &
    pids+=("$!")
  done
  local rc=0
  for pid in "${pids[@]}"; do wait "$pid" || rc=$?; done
  [ "$rc" -eq 0 ] || return "$rc"
  cat "$output"/shards/*.jsonl > "$output/predictions.jsonl"
  "$PYTHON" src/evaluate.py "$output/predictions.jsonl" --output "$output/metrics.json" \
    > "$output/evaluate.log" 2>&1
}

for lr in 1e-4 2e-4; do
  echo "[$(date -Is)] evaluating lr=$lr with $SHARDS shards per GPU"
